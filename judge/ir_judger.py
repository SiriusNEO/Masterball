...
for judge in judge_list:
    ...
    wrap = os.popen("diff {file1} {file2} -w".format(file1 = output_file, file2 = std_file))
    info = wrap.readlines()

    if len(info) == 0:
        print("\033[32m[Success] [std]: in {testpoint} \033[0m".format(testpoint = code_file + ", point " + str(cnt)))
    else:
        print("\033[31m[Failed] [std]: in {testpoint} \033[0m".format(testpoint = code_file + ", point " + str(cnt)))
        print("info: ", info)
        # exit(-1)
